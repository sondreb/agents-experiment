...
- The developer is the sole maintainer and a senior expert in Angular, TypeScript, and Nostr. Communicate at that level: skip beginner explanations, hedging, and filler. Lead with the answer or the change, not the reasoning.
- Don't ask clarifying questions unless a wrong guess would be destructive or hard to reverse. State your assumptions in one line and proceed.
- Challenge the task when a clearly better approach exists: propose it briefly with the tradeoff, then proceed with the best option unless told otherwise.
- Innovation is welcome. Suggest bold improvements when they meaningfully benefit the app, but keep the diff you actually make focused on the task.

## Non-negotiables (read first)

1. Nostr timestamps are in **SECONDS**: `Math.floor(Date.now() / 1000)`.
2. Standalone components, signals, `input()`/`output()`, `inject()`, `OnPush`. No NgModules, no `@Input`/`@Output` decorators, no constructor injection.
3. Native control flow (`@if`/`@for`/`@switch`) - never `*ngIf`/`*ngFor`/`*ngSwitch`.
4. `fetch`, never HttpClient.
5. Material 3 CSS variables (`--mat-sys-*`) - never hardcoded colors, never `font-weight`, never `color="primary"`.
6. `CustomDialogComponent` - never Material dialogs, never native `confirm()`.
7. Minimal, surgical diffs. Don't refactor or "improve" unrelated code.
8. Never claim done without evidence: run the smallest relevant lint/build/test and quote the actual result.

# Coding preferences (General)

- Keep things simple. Channel "yagni" energy unless told otherwise.
- Typesafety is useful, take advantage of it.
- Don't be scared to propose bold ideas if they can meaningfully benefit the app.
...
- Bias to action: when the intent is clear, implement fully rather than proposing options.
- Make the smallest change that **fully** solves the problem. Complete beats minimal-but-incomplete.
- Parallelize independent work (file reads, searches) instead of doing them sequentially.
- Before writing code, search the codebase for existing patterns and match them. Don't introduce new patterns, utilities, or dependencies when an equivalent already exists.
- Use the most targeted validation command that covers the change (single spec file, `--grep` tag). Escalate to full suites only when the targeted run shows a need.
- If the first approach fails, try an alternative before declaring something impossible.
- Don't create documentation or markdown files unless explicitly asked.
- Never commit secrets; never commit without being asked.
